...
@if(Auth::user()->authorizepermisos(['Cargo', 'Ver']) && Auth::user()->authorizepermisos(['Cargo', 'Crear']) )
...
<div style="padding: 6rem; padding-top:0.5rem;">
  <div class="row d-flex justify-content-center mb-3">
    <div class="col-lg-6 col-sm-12 d-flex align-items-center justify-content-center">
      <h3 class="text-center titulo1">Editar de Cargo</h3>
      <a href="{{ url('/') }}">
  ...
  <form method="POST" enctype="multipart/form-data" action="{{ route('cargo.update',$cargo[0]->id) }}">
    @csrf
    <div class="row">
      <div class="col-12 borde1">
        <h3 class="mt-3">Datos de departamento</h3>
...
<div class="row">
  <div class="col">
          ...
          <input id="name" type="text" value="{{$cargo[0]->name}}" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required autocomplete="name" autofocus>
            @error('name')
  ...
  <div class="col">
          ...
          <input id="descripcion" type="text" value="{{$cargo[0]->description}}" class="form-control @error('descripcion') is-invalid @enderror" name="descripcion" value="{{ old('descripcion') }}" autocomplete="descripcion">
            @error('descripcion')
            <span class="invalid-feedback" role="alert">
              <strong>{{ $message }}</strong>
            </span>
  ...
  <div class="col">
    <label for="salario" class="col col-form-label">
      {{ __('Salario:') }}
    </label>
    <input id="salary" type="text" value="{{$cargo[0]->salary}}" inputmode="decimal" id="float-input" pattern="[0-9]*[.,]?[0-9]*" class="form-control  @error('salary') is-invalid @enderror" name="salary" value="{{ old('salary') }}" autocomplete="salary">
      @error('salary')
      <span class="invalid-feedback" role="alert">
        <strong>{{ $message }}</strong>
      </span>
...
<div class="col">
    <label for="unidad" class="col col-form-label">
        {{ __('Tipo:') }}
      </label>
    <select class="form-select" aria-label="Default select example" name="unidad" id="unidad" required>
        
        @foreach ($unidad as $item)
        @if ($item==$cargo[0]->unidad)
...
<div class="col">
  ...
  <select class="form-select" aria-label="Default select example" name="department" id="department" required>
      ...
      @foreach ($reg_department as $item)
        @if ($item->name==$cargo[0]->nameD)
