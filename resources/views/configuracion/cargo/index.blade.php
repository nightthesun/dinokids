  ...
  <table class="cell-border compact hover" id="myTable" style="width:100%;" >
    <thead>
      <tr>
        ...
        @if(Auth::user()->authorizepermisos(['Cargo', 'Ver informacion']) ) 
        <th>Salario</th>
        <th>Opcines</th>   
        @endif
    ...
    <tbody>
     ...
     <tr>
      ...
      <td>{{$car->nameD}}</td>
      @if(Auth::user()->authorizepermisos(['Cargo', 'Ver informacion']) ) 
      <td>{{$car->salary." ".$car->unidad}}</td> 
      <td class="small-cell">
        @if(Auth::user()->authorizepermisos(['Cargo', 'Editar']) ) 
           <a class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" title="Editar" href="{{ route('cargo.edit',$car->id) }}" >
      ...
      <form method="POST" enctype="multipart/form-data" action="{{ route('cargo.destroy', $car->id) }}" class="d-inline">
        @csrf 
      
            <button type="button" class="btn btn-danger btn-sm text-white" title="Activar" data-bs-toggle="modal" data-bs-target="#Delete{{$car->id}}" data-toggle="tooltip" data-placement="top">
      ...
      <!-- Modal -->
      <div class="modal fade" id="Delete{{$car->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
      ...
      <div class="modal-body">
        Realmente decea eliminar el cargo de
        {{$car->name}}.
        <div class="mb-3">
