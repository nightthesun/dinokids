...
<div style="padding: 6rem; padding-top:0.5rem;">
  <div class="row d-flex justify-content-center mb-3">
    <div class="col-lg-6 col-sm-12 d-flex align-items-center justify-content-center">
      <h3 class="text-center titulo1">Edición de sucursal </h3>
      <a href="{{ url('/') }}">
  ...
  <form method="POST" enctype="multipart/form-data" action="{{ route('sucursal.update',$branch[0]->id) }}">
    @method('PUT')
    @csrf
...
<div class="row">
  <div class="col">
          ...
          <input id="name" type="text" value="{{$branch[0]->nameDres}}" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required autocomplete="name" autofocus>
            @error('name')
  ...
  <div class="col">
          ...
          <input id="descripcion" value="{{$branch[0]->descripS}}" type="text" class="form-control @error('descripcion') is-invalid @enderror" name="descripcion" value="{{ old('descripcion') }}" autocomplete="descripcion">
            @error('descripcion')
            <span class="invalid-feedback" role="alert">
              <strong>{{ $message }}</strong>
            </span>
